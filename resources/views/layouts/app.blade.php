...
/* ══ NAVBAR ══ */
.nav{background:rgba(255,255,255,.97);backdrop-filter:blur(14px);border-bottom:1px solid var(--bds);padding:0 var(--side-pad);position:sticky;top:0;z-index:200;width:100%}
...
/* ══ TABS ROW ══ */
.nav-tabs{display:flex;gap:4px;width:100%;height:46px;border-top:1px solid var(--bds);overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch;align-items:center}
.nav-tabs::-webkit-scrollbar{display:none}
.nt{padding:8px 16px;border-radius:var(--rF);font-weight:700;font-size:14px;color:var(--tm);border:none;background:none;font-family:var(--font);cursor:pointer;transition:.2s;text-decoration:none;display:inline-flex;align-items:center;white-space:nowrap;flex-shrink:0}
.nt:hover,.nt.on{color:var(--p);background:var(--pl)}
.nt.bk{background:var(--p);color:#fff;padding:8px 20px}.nt.bk:hover{background:var(--pd)}
...
.fp{background:var(--card);border:1.5px solid var(--bdr);border-radius:var(--r22);padding:22px;position:sticky;top:112px}
...
/* ══════════════════════════════════════════
   RESPONSIVE — لابتوب كبير (1200px+)
   المحتوى يتمدد ويكون في المنتصف
══════════════════════════════════════════ */
