...
/* ── Doctor SVG wrapper - أصغر من قبل ── */
.hero-vis{display:flex;align-items:center;justify-content:center}
.hero-vis svg{
  width:260px;height:260px;
  filter:drop-shadow(0 14px 28px rgba(90,174,122,0.18));
...
<div class="sec" id="section-services">
  <div class="si">
    <div class="sh">
      ...
      <p>رعاية صحية متكاملة في منصة واحدة — ثلاث خدمات رئيسية لتوفير رعاية شاملة وسهلة الوصول</p>
    </div>
    ...
    <div class="svc-grid">
      ...
      {{-- خدمة 2: الزيارات المنزلية (featured) --}}
      <div class="svc-card svc-card-featured">
        ...
        <div class="svc-icon svc-icon-featured">
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
...
<div class="sec" id="section-join" style="background:var(--card)">
  <div class="si">
    <div class="sh">
      ...
      <h2>رسالتنا ودعوة للتعاون</h2>
      <p>نسعى لتقديم رعاية صحية ممتازة بأقل تكلفة ممكنة — نرحب بشراكتكم ندعو الشركاء للانضمام في رحلة تطوير الرعاية الصحية في ليبيا.</p>
    </div>
...
// يمكنك استدعاء scrollToSection('pharmacies') من أي مكان
function scrollToSection(sectionId) {
    const el = document.getElementById('section-' + sectionId);
    if (!el) return;
    el.scrollIntoView({ behavior: 'smooth', block: 'start' });
    // بعد الوصول أضف تأثير الـ highlight
    setTimeout(function() {
        // إزالة الـ class من كل العناصر أولاً
...
// بطاقات الصيدليات: عند الضغط على صيدلية محددة تنتقل لصفحة الصيدليات
// يمكن تعديل هذا لاحقاً لعرض تفاصيل الصيدلية
...
// دعم #anchor في الـ URL للانتقال المباشر
(function() {
    const hash = window.location.hash.replace('#','');
    if (hash) {
        setTimeout(function(){ scrollToSection(hash); }, 400);
    }
})();
</script>
@endsection
