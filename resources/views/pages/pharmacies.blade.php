...
.ph-grid-big{display:grid;grid-template-columns:repeat(3,1fr);gap:18px}
...
.ph-status{position:absolute;top:10px;left:12px;display:flex;align-items:center;gap:5px;font-size:11px;font-weight:700;padding:4px 10px;border-radius:var(--rF);background:#fff;box-shadow:var(--s1)}
...
.close-btn{width:34px;height:34px;border-radius:50%;background:var(--bg);border:1.5px solid var(--bdr);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:18px;color:var(--tm)}
...
.whatsapp-btn{width:100%;padding:13px;background:#25D366;color:#fff;border:none;border-radius:var(--r8);font-size:14px;font-weight:800;cursor:pointer;font-family:var(--font);display:flex;align-items:center;justify-content:center;gap:8px;margin-top:16px;text-decoration:none}
.chat-section{background:var(--bg);border-radius:var(--r14);padding:16px;margin-top:16px;border:1px solid var(--bds)}
...
@media(max-width:768px){
  ...
  .ph-search{grid-template-columns:1fr!important;gap:8px}
  .ph-grid-big{grid-template-columns:1fr!important}
  .med-grid{grid-template-columns:repeat(2,1fr)!important}
}
</style>
@endsection
...
<div class="si" style="padding-top:28px;padding-bottom:60px">
  ...
  <div class="ph-hero">
    <div style="position:relative;z-index:1">
      ...
      <div style="display:flex;flex-direction:column;gap:10px;margin-top:16px">
        <div class="ph-hero-feat"><svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>تواصل مباشر مع الصيدلي</div>
        <div class="ph-hero-feat"><svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>معرفة توفر الدواء فوراً</div>
        <div class="ph-hero-feat"><svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>بدائل الأدوية المتاحة</div>
      </div>
    </div>
    <div style="position:relative;z-index:1">
  ...
  <div class="ph-search">
    ...
    <button class="filter-btn on" id="filter-all" onclick="setFilter('all',this)">الكل</button>
...
function renderPharmacies(list) {
  ...
  list.forEach(function(p) {
    html += '<div class="ph-card-big" data-name="' + p.name + '" onclick="openModal(' + p.id + ')">';
    ...
    html += (p.open ? 'مفتوحة' : 'مغلقة') + '</div></div>';
    ...
    html += '<button class="ph-btn ph-btn-o" onclick="event.stopPropagation();openModal(' + p.id + ')">تواصل مع الصيدلي</button>';
    html += '</div></div></div>';
  });
...
function openModal(id) {
  ...
  if (!chatMsgs[id]) chatMsgs[id] = [{from:'ph',text:'مرحباً! كيف يمكنني مساعدتك اليوم؟',time:'الآن'}];

  let medsHtml = '';
  p.meds.forEach(function(m){
    medsHtml += '<div class="med-item"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="4"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg><p>' + m + '</p><span>متوفر</span></div>';
  });
...
function sendPhMsg(id) {
  ...
  updateChat(id);
  setTimeout(function(){
    const replies = ['نعم، هذا الدواء متوفر لدينا 💊','يمكنك المرور لاستلامه في أي وقت','سأتحقق من توفره وأعلمك قريباً','هل لديك وصفة طبية لهذا الدواء؟'];
